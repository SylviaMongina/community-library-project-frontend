import React from 'react'

function Footer() {
  return (
    <div id="contact" className="footer">
       <a id="jumpHere"></a>

<div className="container">
  <div className="row">
    <div className="footer-col-3">
      <h3>Contacts</h3>
     <ul>
      <li> +254729618721 Kiserian </li>
      <li> +254712345678 Rongai</li>

      <li>email <a href="">sylviamonginab@gmail.com</a></li>
     </ul>
    </div>
    <div className="footer-col-3">
      <h3>Location</h3>
      <p>We are located along Magadi Road Dolpin Plaza <br/>Ground Floor G10</p>
     <ul>
      
     </ul>
    </div>
    <div className="footer-col-4">
      <h3>Follow us</h3>
     <ul>
      <li><a href="https://www.facebook.com/">Facebook</a></li>
      <li><a href="https://twitter.com/i/flow/login">Twitter</a></li>
      <li><a href="https://www.instagram.com/accounts/login/">Instagram</a></li>
      <li><a href="https://www.youtube.com/">YouTube</a></li>
     </ul>
    </div>
  </div>
  
  <p className="copyright" >copright@2023-Sylvia Mongina</p>
</div>

    </div>
  )
}

export default Footer;